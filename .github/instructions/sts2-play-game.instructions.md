...
- **动画间隔**：`combat_batch` 中每个操作之间需要等待游戏动画完成。MCP 已内置 300ms 延迟，但复杂动画（如大量抽牌、AOE、连锁触发）可能需要更长时间。如果操作不生效，改为逐步手动执行。
- **多张选牌从右到左**：在 `hand_select`（消耗/弃牌/升级选择）中需要选多张牌时，**必须从最高索引开始选取**，防止前面的牌被选走后后续索引偏移。
- **⭐ 抽牌/随机效果自动暂停 (Auto-Pause)**：`combat_batch` 现在会**自动检测手牌中含有 `draws_cards` 或 `has_randomness` 效果标签的卡牌**（通过 `effects` 字段）。当这类卡牌被打出且后面还有 `play_card` 操作时，batch 会**自动暂停**，返回：
  - 已执行的操作结果
  - 暂停原因和触发卡牌名称
  - **剩余未执行的 action 列表**（供参考）
  - **最新的游戏状态**（含新抽到的手牌）
  - ⚡ 你**不需要额外调用 `get_game_state()`**，直接用返回的状态重新规划！
- **最佳实践**：含 `draws_cards` 效果的卡牌（如燃烧契约、祭品、战斗冥想）应作为 batch 中**最后一张 play_card**（之后只放 end_turn 或不放）。这样 batch 不会被暂停。
- **顺序警告**：batch 现在会**自动检测不优顺序**。如果减益牌（易伤/虚弱）排在攻击牌之后，会在结果中插入 `⚠️ SUBOPTIMAL ORDER` 警告。**看到此警告应取消当前计划并重新排序**。
